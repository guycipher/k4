...
void* benchmark_rocksdb(void* arg);
void* benchmark_lmdb(void* arg);
void* benchmark_k4(void* arg);
...
int main() {
    ...
    // Create threads for RocksDB benchmark
    for (i = 0; i < NUM_THREADS; i++) {
        pthread_create(&threads[i], NULL, benchmark_rocksdb, NULL);
    }
    for (i = 0; i < NUM_THREADS; i++) {
        pthread_join(threads[i], NULL);
    }

    // Create threads for LMDB benchmark
    for (i = 0; i < NUM_THREADS; i++) {
        pthread_create(&threads[i], NULL, benchmark_lmdb, NULL);
    }
    for (i = 0; i < NUM_THREADS; i++) {
        pthread_join(threads[i], NULL);
    }

    // Create threads for K4 benchmark
    for (i = 0; i < NUM_THREADS; i++) {
        pthread_create(&threads[i], NULL, benchmark_k4, NULL);
    }
    for (i = 0; i < NUM_THREADS; i++) {
        pthread_join(threads[i], NULL);
    }
